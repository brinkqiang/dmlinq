...
namespace linq {
    ...
    template<
        class OuterIterator,
        class Selector,
        class SelectorRangeRef = typename std::invoke_result<Selector(typename boost::iterator_reference<OuterIterator>::type)>::type,
    ...
    {
        static_assert(is_bindable_range<typename std::invoke_result<Selector(typename boost::iterator_reference<Iterator>::type)>::type>::value, "Ranges returned from select_many selector must be bindable");
